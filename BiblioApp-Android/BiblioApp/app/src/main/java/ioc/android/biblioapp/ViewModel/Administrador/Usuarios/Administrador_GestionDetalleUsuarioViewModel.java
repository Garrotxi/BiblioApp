...
import static ioc.android.biblioapp.ViewModel.Administrador.Usuarios.Administrador_GestionUsuariosViewModel.conseguirUsuarioPorNombre;
...
public class Administrador_GestionDetalleUsuarioViewModel extends ViewModel {
    ...
    public LiveData<Usuari> modificarUsuario(Administrador_GestionDetalleUsuarioViewModel administrador_gestionDetalleUsuarioViewModel, Context context, String token, Usuari usuari) {
        mensaje = new MutableLiveData<>();
        administrador_gestionDetalleUsuarioViewModel.modificarUsuarios(usuari, token, context).observe((LifecycleOwner) context, new Observer<Usuari>() {
            @Override
    ...
    public LiveData<String> borrarUsuario(Administrador_GestionDetalleUsuarioViewModel administrador_gestionDetalleUsuarioViewModel, Context context, String token, Usuari usuari) {
        ...
        administrador_gestionDetalleUsuarioViewModel.borraUser(usuari, token, context).observe((LifecycleOwner) context, new Observer<String>() {
            @Override
     ...
     * @return respuesta API
     */
    public LiveData<String> borraUser(Usuari usuari, String token, Context context) {
        if (mText2==null) {
            mText2 = biblioAppRepo.borraUsuario(usuari, token, context);
        }
        return mText2;
    }
}
