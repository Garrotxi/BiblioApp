...
public class Administrador_DetalleAutorViewModel extends ViewModel {
    ...
    public LiveData<Autor> modificarAutor(Administrador_DetalleAutorViewModel administradorDetalleAutorViewModel, Context context, String token, Autor autor) {
        mensaje = new MutableLiveData<>();
        administradorDetalleAutorViewModel.modificarAutores(autor, token, context).observe((LifecycleOwner) context, new Observer<Autor>() {
            @Override
    ...
    public LiveData<Autor> modificarAutores(Autor autor, String token, Context context) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.modificarAutor(autor, token, context);
        }
    ...
    public LiveData<String> borrarAutor(Administrador_DetalleAutorViewModel administrador_detalleAutorViewModel, Context context, String token, Autor autor) {
        ...
        administrador_detalleAutorViewModel.borraAuthor(autor, token,context).observe((LifecycleOwner) context, new Observer<String>() {
            @Override
     ...
     * @return llamada a repositorio que devuelve resultado
     */
    public LiveData<String> borraAuthor(Autor autor, String token, Context context) {
        if (mText2==null) {
            mText2 = biblioAppRepo.borraAutor(autor, token, context);
        }
        return mText2;
    }
}
