...
public class Administrador_NuevoAutorViewModel {
    ...
    public MutableLiveData<String> nuevoAutor(Autor autor, Context context, Administrador_NuevoAutorViewModel administrador_nuevoAutorViewModel, String token){
        ...
        administrador_nuevoAutorViewModel.nuevoAuthor(autor,token).observe((LifecycleOwner) context, new Observer<Registro>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Registro> nuevoAuthor(Autor autor, String token) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.añadeAutor(autor, token);
        }
