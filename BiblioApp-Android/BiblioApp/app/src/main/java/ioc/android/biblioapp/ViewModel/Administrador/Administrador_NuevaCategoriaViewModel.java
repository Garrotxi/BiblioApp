...
public class Administrador_NuevaCategoriaViewModel {
    ...
    public MutableLiveData<String> nuevaCategoria(Categoria categoria, Context context, Administrador_NuevaCategoriaViewModel administrador_nuevaCategoriaViewModel, String token){
        ...
        administrador_nuevaCategoriaViewModel.nuevaCathegory(categoria,token).observe((LifecycleOwner) context, new Observer<Registro>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Registro> nuevaCathegory(Categoria categoria, String token) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.añadeCategoria(categoria, token);
        }
