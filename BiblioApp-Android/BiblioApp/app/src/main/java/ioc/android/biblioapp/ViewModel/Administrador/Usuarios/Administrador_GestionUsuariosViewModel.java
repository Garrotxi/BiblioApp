...
public class Administrador_GestionUsuariosViewModel extends ViewModel {
    ...
    public LiveData<Collection<Usuari>> getListaUsuarios(Administrador_GestionUsuariosViewModel administrador_gestionUsuariosViewModel, Context context, String token) {
        ...
        administrador_gestionUsuariosViewModel.ConseguirUsuariosRepositorio(token, context).observe((LifecycleOwner) context, new Observer<Collection<Usuari>>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Collection<Usuari>> ConseguirUsuariosRepositorio(String token, Context context) {
        if (mutableLiveData == null) {
            mutableLiveData = biblioAppRepo.pideUsuarios(token, context);
        }
