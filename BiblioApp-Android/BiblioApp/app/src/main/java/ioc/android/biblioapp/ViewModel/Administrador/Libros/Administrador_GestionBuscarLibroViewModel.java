...
public class Administrador_GestionBuscarLibroViewModel extends ViewModel {
    ...
    public LiveData<Llibre> buscarLibroId(Administrador_GestionBuscarLibroViewModel administrador_gestionBuscarLibroViewModel, Context context, String token, Llibre llibre) {
        ...
        administrador_gestionBuscarLibroViewModel.buscaIdBook(llibre, token,context).observe((LifecycleOwner) context, new Observer<Llibre>() {
            @Override
     ...
     * @return
     */
    public LiveData<Llibre> buscaIdBook(Llibre llibre, String token, Context context) {
        if (mensaje2==null) {
            mensaje2 = biblioAppRepo.buscaLibroId(llibre, token, context);
        }
        return mensaje2;
    }
    ...
    public LiveData<Llibre> buscarLibroTitulo(Administrador_GestionBuscarLibroViewModel administrador_gestionBuscarLibroViewModel, Context context, String token, Llibre llibre) {
        mensaje = new MutableLiveData<>();
        administrador_gestionBuscarLibroViewModel.buscaTituloBook(llibre, token, context).observe((LifecycleOwner) context, new Observer<Llibre>() {
            @Override
     ...
     * @return llamada a repositorio que devuelve resultado
     */
    public LiveData<Llibre> buscaTituloBook(Llibre llibre, String token, Context context) {
        if (mensaje2==null) {
            mensaje2 = biblioAppRepo.buscaLibroTitulo(llibre, token, context);
        }
        return mensaje2;
    }
}
