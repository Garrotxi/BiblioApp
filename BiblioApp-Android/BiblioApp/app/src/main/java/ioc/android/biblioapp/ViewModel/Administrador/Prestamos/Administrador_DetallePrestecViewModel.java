...
public class Administrador_DetallePrestecViewModel extends ViewModel {
    ...
    public LiveData<Prestec> modificarPrestec(Administrador_DetallePrestecViewModel administrador_detallePrestecViewModel, Context context, String token, Prestec prestec) {
        mensaje = new MutableLiveData<>();
        administrador_detallePrestecViewModel.modificarPrestec(prestec, token,context).observe((LifecycleOwner) context, new Observer<Prestec>() {
            @Override
    ...
    public LiveData<Prestec> modificarPrestec(Prestec prestec, String token, Context context) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.modificarPrestec(prestec, token, context);
        }
    ...
    public LiveData<String> borrarPrestec(Administrador_DetallePrestecViewModel administrador_detallePrestecViewModel, Context context, String token, Prestec prestec) {
        ...
        administrador_detallePrestecViewModel.borraLoan(prestec, token, context).observe((LifecycleOwner) context, new Observer<String>() {
            @Override
     ...
     * @return llamada a repositorio que devuelve resultado
     */
    public LiveData<String> borraLoan(Prestec prestec, String token, Context context) {
        if (mText2==null) {
            mText2 = biblioAppRepo.borraPrestec(prestec, token, context);
        }
        return mText2;
    }
}
