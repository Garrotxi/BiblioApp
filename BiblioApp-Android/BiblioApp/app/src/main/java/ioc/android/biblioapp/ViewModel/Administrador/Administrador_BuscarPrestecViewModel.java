...
public class Administrador_BuscarPrestecViewModel extends ViewModel {
    ...
    public LiveData<Prestec> buscarPrestecId(Administrador_BuscarPrestecViewModel administrador_gestionBuscarLibroViewModel, Context context, String token, Prestec prestec) {
        ...
        administrador_gestionBuscarLibroViewModel.buscaIdLoan(prestec, token).observe((LifecycleOwner) context, new Observer<Prestec>() {
            @Override
     ...
     * @return
     */
    public LiveData<Prestec> buscaIdLoan(Prestec prestec, String token) {
        if (mensaje2==null) {
            mensaje2 = biblioAppRepo.buscaPrestecId(prestec, token);
        }
        return mensaje2;
    }
