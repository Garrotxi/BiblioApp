...
public class Administrador_NuevoPrestecViewModel {
    ...
    public MutableLiveData<String> nuevoPrestec (Prestec prestec, Context context, Administrador_NuevoPrestecViewModel administrador_nuevoPrestecViewModel, String token){
        ...
        administrador_nuevoPrestecViewModel.conseguirRegistroRepositorio(prestec,token).observe((LifecycleOwner) context, new Observer<Registro>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Registro> conseguirRegistroRepositorio(Prestec prestec, String token) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.añadePrestec(prestec, token);
        }
