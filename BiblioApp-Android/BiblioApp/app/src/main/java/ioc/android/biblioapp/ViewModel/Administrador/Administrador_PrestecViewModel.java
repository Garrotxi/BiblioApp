...
public class Administrador_PrestecViewModel extends ViewModel {
    ...
    public LiveData<Collection<Prestec>> getListaPrestecs(Administrador_PrestecViewModel administrador_prestecViewModel, Context context, String token) {
        ...
        administrador_prestecViewModel.ConseguirPrestecsRepositorio(token).observe((LifecycleOwner) context, new Observer<Collection<Prestec>>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Collection<Prestec>> ConseguirPrestecsRepositorio(String token) {
        if (mutableLiveData == null) {
            mutableLiveData = biblioAppRepo.pidePrestecs(token);
        }
