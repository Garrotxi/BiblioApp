...
public class Administrador_CategoriasViewModel extends ViewModel {
    ...
    public LiveData<Collection<Categoria>> getListaCategorias(Administrador_CategoriasViewModel administrador_categoriasViewModel, Context context, String token) {
        ...
        administrador_categoriasViewModel.ConseguirCategoriasRepositorio(token).observe((LifecycleOwner) context, new Observer<Collection<Categoria>>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Collection<Categoria>> ConseguirCategoriasRepositorio(String token) {
        if (mutableLiveData == null) {
            mutableLiveData = biblioAppRepo.pideCategorias(token);
        }
