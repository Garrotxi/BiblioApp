...
public class Administrador_DetalleCategoriaViewModel extends ViewModel {
    ...
    public LiveData<Categoria> modificarCategoria(Administrador_DetalleCategoriaViewModel administrador_detalleCategoriaViewModel, Context context, String token, Categoria categoria) {
        mensaje = new MutableLiveData<>();
        administrador_detalleCategoriaViewModel.modificarCategorias(categoria, token).observe((LifecycleOwner) context, new Observer<Categoria>() {
            @Override
    ...
    public LiveData<Categoria> modificarCategorias(Categoria categoria, String token) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.modificarCategoria(categoria, token);
        }
    ...
    public LiveData<String> borrarCategoria(Administrador_DetalleCategoriaViewModel administrador_detalleCategoriaViewModel, Context context, String token, Categoria categoria) {
        ...
        administrador_detalleCategoriaViewModel.borraCathegory(categoria, token).observe((LifecycleOwner) context, new Observer<String>() {
            @Override
     ...
     * @return llamada a repositorio que devuelve resultado
     */
    public LiveData<String> borraCathegory(Categoria categoria, String token) {
        if (mText2==null) {
            mText2 = biblioAppRepo.borraCategoria(categoria, token);
        }
        return mText2;
    }
}
