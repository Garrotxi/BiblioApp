...
public class Administrador_AutoresViewModel extends ViewModel {
    ...
    public LiveData<Collection<Autor>> getListaAutores(Administrador_AutoresViewModel administrador_autoresViewModel, Context context, String token) {
        ...
        administrador_autoresViewModel.ConseguirAutoresRepositorio(token, context).observe((LifecycleOwner) context, new Observer<Collection<Autor>>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Collection<Autor>> ConseguirAutoresRepositorio(String token, Context context) {
        if (mutableLiveData == null) {
            mutableLiveData = biblioAppRepo.pideAutores(token, context);
        }
