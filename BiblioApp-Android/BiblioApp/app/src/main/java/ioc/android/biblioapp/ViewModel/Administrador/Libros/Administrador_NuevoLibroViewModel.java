...
public class Administrador_NuevoLibroViewModel {
    ...
    public MutableLiveData<String> nuevoLibro (Llibre libro, Context context, Administrador_NuevoLibroViewModel administrador_nuevoLibroViewModel, String token){
        ...
        administrador_nuevoLibroViewModel.conseguirRegistroRepositorio(libro,token, context).observe((LifecycleOwner) context, new Observer<Registro>() {//Generamos un observer a la espera de la consulta con el repositorio
            @Override
     ...
     */
    public LiveData<Registro> conseguirRegistroRepositorio(Llibre libro, String token, Context context) {
        if (mutableLiveData==null) {
            mutableLiveData = biblioAppRepo.añadeLibro(libro, token, context);
        }
